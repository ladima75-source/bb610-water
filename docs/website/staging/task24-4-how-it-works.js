...
#how-it-works .how-step-visual{height:138px;display:flex;align-items:flex-start;justify-content:center;margin:0 0 8px;position:relative}
#how-it-works .how-step-title{margin:0 0 7px!important;color:#fff!important;font-size:24px!important;line-height:1.04!important;font-weight:800!important;letter-spacing:-.01em!important;min-height:0!important}
#how-it-works .how-step-copy{margin:0!important;color:#c6d7e0!important;font-size:16px!important;line-height:1.34!important;max-width:285px}
...
#how-it-works .visual-metrics{width:198px;display:grid;gap:6px;align-self:flex-start}
#how-it-works .metric-row{display:grid;grid-template-columns:28px 1fr;align-items:center;gap:8px;padding:5px 8px;border:1px solid rgba(53,183,234,.24);border-radius:8px;background:rgba(10,39,56,.70);min-height:28px}
#how-it-works .metric-row svg{width:22px;height:22px;fill:none;stroke:#33d1ff;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
#how-it-works .metric-row b{display:block;color:#eefaff;font-size:13px;line-height:1.08}
#how-it-works .metric-row span{color:#9bc4d8;font-size:9.5px;line-height:1.08}
...
`;
  ...
  function rebuild(){
    const section=document.querySelector('#how-it-works');if(!section)return;injectStyles();
    const container=section.querySelector(':scope > .container')||section.querySelector('.container');
    const head=section.querySelector('.section-head');
    if(head){const h=head.querySelector('h2');if(h)h.textContent='ВІД ЗАВДАННЯ ДО ПЕРЕВІРЕНОГО РЕЗУЛЬТАТУ';const p=head.querySelector('p');if(p)p.textContent='BB610 WATER не просто відкриває клапан. Система виконує заданий сценарій, вимірює процес і порівнює результат із завданням.'}
    ...
    if(steps){steps.className='how-process-flow';steps.innerHTML=data.map((d,i)=>`<article><div class="how-step-headrow"><span class="how-step-number">${d[0]}</span><div class="how-step-orb">${ico[d[1]]}</div></div><div class="how-step-visual">${visual(d[1])}</div><h3 class="how-step-title">${d[2]}</h3><p class="how-step-copy">${d[3]}</p>${i<3?'<div class="how-connector" aria-hidden="true"></div>':''}</article>`).join('')}
    const slot=section.querySelector('#connection-scheme-slot');if(slot)slot.innerHTML='';
    let panel=section.querySelector('.how-process-panel');
    if(!panel&&steps){panel=document.createElement('div');panel.className='how-process-panel';const top=document.createElement('div');top.className='how-process-topline';top.innerHTML=`<p class="how-process-kicker">ЄДИНИЙ РОБОЧИЙ ЦИКЛ</p><div class="how-process-topline-line"></div>`;panel.append(top,steps);(container||section).appendChild(panel)}
  }
